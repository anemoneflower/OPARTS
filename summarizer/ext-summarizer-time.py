...
def main():
    ...
    times = {'pororo-ex':[], 'kobert-ex':[]}
    ext_scores = {'pororo-ex':[], 'kobert-ex':[]}
    rouge_scores = {'pororo-ex':[], 'kobert-ex':[]}

    with open("./ext-dataset.json", "r") as f:
        data = json.load(f)

    # SUMMARY
    for i, idx in enumerate(list(data.keys())):
    # for i, idx in enumerate( list(data.keys())[:5]):

        text = " ".join(data[idx]['sentences'])
        fulltext = data[idx]['sentences']
        ext_reference = data[idx]['summaries'] # e.g. [1, 2, 4]
        abs_reference = " ".join([data[idx]['sentences'][i] for i in ext_reference])
        ...
        if i % 50 == 0:
            print("DONE [{}/{}, {}%]".format(i, len(data.keys()), i/len(data.keys())*100))

    for key in times.keys():
        print(key, np.mean(times[key]), [round(i, 3) for i in times[key]])

    with open("./time-test-results/time-ext.json", "w") as f:
        json.dump(times, f, indent=4)
    with open("./time-test-results/ext-scores-ext.json", "w") as f:
        json.dump(ext_scores, f, indent=4)
    with open("./time-test-results/rouge-scores-ext.json", "w") as f:
