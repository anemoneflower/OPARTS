...
def main():
    ...
    text1 = '조금 분들이세요. 이런 그 문제가 있는 것 같고요. 그런데 그것보다 더 근본에는 저는. 이준석 대표는. 정당이 집권한다라고 하는 자기 스타일이 굉장히 분명한 것 같아요. 자기 목표가 당이 집권한다. 그런데 윤석열 후보나 다른 체제 이런 분들은 후보가 당선되는 거다. 정당이 집권한다. 보다는 보면 정당은.'
    while(1):
      text = input("text: ");
      pororo_ab_res, pororo_ex_res, kobart_ab_res, kobert_ex_res = get_summaries(text)
