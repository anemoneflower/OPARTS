...
def main():
    ...
    for idx in list(data.keys())[:5]:
        ...
        start = time.time()
        hypothesis = pororo_abstractive_model(text)
        times['pororo-ab'].append(time.time()-start)
        print('\n', hypothesis); summaries.append(hypothesis)
        rouge_score = get_rouge_score(hypothesis, abs_reference) if hypothesis!="" else 0
        rouge_scores['pororo-ab'].append(rouge_score)

        start = time.time()
        hypothesis = kobart_summarizing_model(text)
        times['kobart-ab'].append(time.time()-start)
        print('\n', hypothesis); summaries.append(hypothesis)
        rouge_score = get_rouge_score(hypothesis, abs_reference) if hypothesis!="" else 0; rouge_scores['kobart-ab'].append(rouge_score)

        start = time.time()
        hypothesis = pororo_extractive_model(text) 
        times['pororo-ex'].append(time.time()-start)
        print('\n', hypothesis); summaries.append(hypothesis)
        rouge_score = get_rouge_score(hypothesis, abs_reference) if hypothesis!="" else 0; rouge_scores['pororo-ex'].append(rouge_score)
        ext_score = get_ext_score(hypothesis, fulltext, ext_reference) if hypothesis!="" else 0; ext_scores['pororo-ex'].append(ext_score)
    
        start = time.time()
        hypothesis = kobert_summarizing_model(text)
        times['kobert-ex'].append(time.time()-start)
        print('\n', hypothesis); summaries.append(hypothesis)
        rouge_score = get_rouge_score(hypothesis, abs_reference) if hypothesis!="" else 0; rouge_scores['kobert-ex'].append(rouge_score)
        ext_score = get_ext_score(hypothesis, fulltext, ext_reference) if hypothesis!="" else 0; ext_scores['kobert-ex'].append(ext_score)

        start = time.time()
        hypothesis = textRank_extractive_model([text])
        times['textrank'].append(time.time()-start)
        print('\n', hypothesis)
        rouge_score = get_rouge_score(hypothesis, abs_reference) if hypothesis!="" else 0; rouge_scores['textrank'].append(rouge_score)
        ext_score = get_ext_score(hypothesis, fulltext, ext_reference) if hypothesis!="" else 0; ext_scores['textrank'].append(ext_score)
    ...
    with open("./time-test-results/time.json", "w") as f:
        json.dump(times, f, indent=4)
    with open("./time-test-results/ext-scores.json", "w") as f:
        json.dump(ext_scores, f, indent=4)
    with open("./time-test-results/rouge-scores.json", "w") as f:
